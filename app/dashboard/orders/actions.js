...
export async function updateOrderPaymentStatus(orderId, newStatus) {
  ...
  try {
    ...
    logger.info('Server Action: Mise à jour statut commande démarrée', {
      ...
      oldStatus: 'unknown', // Sera récupéré depuis la DB
      newStatus,
      component: 'orders_server_action',
      action: 'update_start',
    });
    ...
    try {
      ...
      logger.debug('Server Action: Invalidation du cache après mise à jour', {
        requestId,
        orderId: orderIdNumber,
        component: 'orders_server_action',
        action: 'cache_invalidation_start',
      ...
      const cacheInvalidated = await invalidateOrdersCache(
        orderIdNumber.toString(),
      );

      if (cacheInvalidated) {
        logger.debug('Server Action: Cache invalidé avec succès', {
          requestId,
          orderId: orderIdNumber,
          component: 'orders_server_action',
          action: 'cache_invalidation_success',
      ...
      // ===== ÉTAPE 7: LOGGING DE SUCCÈS ET NETTOYAGE =====
      const responseTime = Date.now() - startTime;
